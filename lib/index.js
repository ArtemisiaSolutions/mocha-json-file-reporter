...
function JSONFileReporter(runner) {
  ...
  runner.on('end', function(){
    ...
    if(process.env.JSON_REPORT_STDOUT_DISABLED !== 'true') {
    ...
    if(process.env.REPORT_FILENAME) {
      filename = process.env.REPORT_FILENAME
    }

    try {
        util.print("\nGenerating "+filename+" file")
        var out  = fs.openSync(path+"/"+filename, "w");

        fs.writeSync(out, jsonOutput);
        fs.close(out);
        util.print("\nGenerating "+filename+" file complete in "+path+"\n")
    } catch (error) {
